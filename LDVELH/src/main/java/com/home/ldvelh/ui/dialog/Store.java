...
public abstract class Store<T extends ListItem> extends AdventureDialog implements Observer {
	...
	private final boolean syncWithList;
	...
	Store(Context context, Object data, int layoutResId, IntValueHolder srcCurrency, ListValueHolder<T> list, boolean syncWithList) {
		super(context, data);
		this.layoutResId = layoutResId;
		this.srcCurrency = srcCurrency;
		this.currency = new IntValueHolder(srcCurrency);
		this.syncWithList = syncWithList;
		this.list = list;
	}
	...
	void acquireItems() {
		for (ShopPickerItem shopItem : getAllShopItems()) {
			if (syncWithList) {
				T existingItem = list.find(shopItem.getName());
				if (existingItem != null) {
					existingItem.add(shopItem.getQuantity() - existingItem.getQuantity());
				} else {
	...
	private void initPickers(int remainingCurrency) {
		for (ShopPickerItem shopItem : getAllShopItems()) {
			...
			if (syncWithList) {
				T existingItem = list.find(shopItem.getName());
				if (existingItem != null) {
					initValue = existingItem.getQuantity();
				}
			}
