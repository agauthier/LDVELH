...
public class ListValueHolder<T extends ListItem> extends ValueHolder<List<T>>implements Iterable<T> {
	...
	@SuppressWarnings("UnusedReturnValue") public T add(String itemName, Object data) {
	...
	@SuppressWarnings("WeakerAccess") public T add(ItemAndQuantity itemAndQty, Object data) {
	...
	@SuppressWarnings("WeakerAccess") public T add(ItemAndQuantity itemAndQty, List<Effect> effects, Object data) {
	...
	public T getItemAbove(T item) {
		int indexOfItem = super.getValue().indexOf(item);
		return (indexOfItem == 0) ? null : super.getValue().get(indexOfItem - 1);
	}

	public T getItemBelow(T item) {
		List<T> list = super.getValue();
		return (list.indexOf(item) == list.size() - 1) ? null : list.get(list.indexOf(item) + 1);
	}
